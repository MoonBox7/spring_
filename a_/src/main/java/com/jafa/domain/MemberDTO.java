...
//@FieldMatch() // @FieldMatch 어노테이션은 비밀번호와 비밀번호 확인 필드의 값이 일치하는지 검증하기 위한 커스텀 어노테이션
public class MemberDTO {
	...
	private String memberName;
					//  8-15자리 문자열 중에 숫자, 알파벳, 특수 문자가 모두 포함
	@Pattern(regexp = "^.*(?=^.{8,15}$)(?=.*\\\\d)(?=.*[a-zA-Z])(?=.*[!@#$%^&+=]).*$") // @Pattern: 어노테이션은 정규식 패턴을 이용하여 문자열이 특정 패턴과 일치하는지 확인할 때 사용됩니다. regexp 파라미터를 이용하여 패턴을 지정하며, 만약 문자열이 패턴과 일치하지 않으면 유효성 검사가 실패하고 메시지를 반환합니다.
